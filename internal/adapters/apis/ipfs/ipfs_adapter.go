...
func (a *IPFSAdapter) GetDnpNameAndServiceName(ctx context.Context, ipfsHash string) (domain.Pkg, error) {
	logger.Debug("[IPFSAdapter] GetDnpNameAndServiceName: ipfsHash=%s", ipfsHash)
	dnpName, err := a.getDnpNameFromHash(ctx, ipfsHash)
	if err != nil {
		logger.Error("[IPFSAdapter] GetDnpNameAndServiceName: failed to get dnpName: %v", err)
	...
	if err != nil {
		logger.Error("[IPFSAdapter] GetDnpNameAndServiceName: failed to get compose service name: %v", err)
		return domain.Pkg{}, fmt.Errorf("failed to get compose service name: %w", err)
	}
	logger.Debug("[IPFSAdapter] GetDnpNameAndServiceName: dnpName=%s serviceName=%s", dnpName, serviceName)
	return domain.Pkg{
		DnpName:     dnpName,
...
func (a *IPFSAdapter) getDnpNameFromHash(ctx context.Context, ipfsHash string) (string, error) {
	...
	logger.Debug("[IPFSAdapter] getDnpNameFromHash: url=%s", url)
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: failed to create request: %v", err)
		return "", err
	}
	resp, err := a.client.Do(req)
	if err != nil {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: request failed: %v", err)
		return "", err
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: non-200 status: %s", resp.Status)
		return "", fmt.Errorf("failed to fetch dappnode_package.json: %s", resp.Status)
	}
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: failed to read body: %v", err)
		return "", err
	}
	var pkg struct {
	...
	if err := json.Unmarshal(body, &pkg); err != nil {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: failed to unmarshal: %v", err)
		return "", err
	}
	if pkg.DnpName == "" {
		logger.Error("[IPFSAdapter] getDnpNameFromHash: dnpName not found in dappnode_package.json")
		return "", fmt.Errorf("dnpName not found in dappnode_package.json")
	}
	logger.Debug("[IPFSAdapter] getDnpNameFromHash: dnpName=%s", pkg.DnpName)
...
func (a *IPFSAdapter) getComposeServiceName(ctx context.Context, ipfsHash string) (string, error) {
	...
	logger.Debug("[IPFSAdapter] getComposeServiceName: url=%s", url)
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		logger.Error("[IPFSAdapter] getComposeServiceName: failed to create request: %v", err)
		return "", err
	}
	resp, err := a.client.Do(req)
	if err != nil {
		logger.Error("[IPFSAdapter] getComposeServiceName: request failed: %v", err)
		return "", err
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		logger.Error("[IPFSAdapter] getComposeServiceName: non-200 status: %s", resp.Status)
	...
	if err := yaml.NewDecoder(resp.Body).Decode(&compose); err != nil {
		logger.Error("[IPFSAdapter] getComposeServiceName: failed to decode yaml: %v", err)
		return "", err
	}
	if len(compose.Services) != 1 {
		logger.Error("[IPFSAdapter] getComposeServiceName: expected 1 service, got %d", len(compose.Services))
		return "", fmt.Errorf("expected exactly one service in docker-compose.yml, got %d", len(compose.Services))
	}
	for name := range compose.Services {
		logger.Debug("[IPFSAdapter] getComposeServiceName: serviceName=%s", name)
		return name, nil
	}
	logger.Error("[IPFSAdapter] getComposeServiceName: no service found in docker-compose.yml")
