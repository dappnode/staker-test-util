...
func (t *TropidatooorAdapter) DataRequest(ctx context.Context, backendName string) (*domain.Mount, error) {
	logger.Debug("[TropidatooorAdapter] DataRequest: backendName=%s", backendName)
	req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("%s/data/request/%s", t.baseURL, backendName), nil)
	if err != nil {
		logger.Error("[TropidatooorAdapter] DataRequest: failed to create request: %v", err)
		return nil, fmt.Errorf("failed to create request: %w", err)
	}

	resp, err := t.client.Do(req)
	if err != nil {
		logger.Error("[TropidatooorAdapter] DataRequest: failed to send request: %v", err)
		return nil, fmt.Errorf("failed to send request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		logger.Error("[TropidatooorAdapter] DataRequest: non-200 status: %s", resp.Status)
	...
	if err := json.NewDecoder(resp.Body).Decode(&dataResponse); err != nil {
		logger.Error("[TropidatooorAdapter] DataRequest: failed to decode response: %v", err)
		return nil, fmt.Errorf("failed to decode response: %w", err)
	}

	logger.Debug("[TropidatooorAdapter] DataRequest: success, dataResponse=%+v", dataResponse)
...
func (t *TropidatooorAdapter) DataRelease(ctx context.Context, uniqueId string) error {
	logger.Debug("[TropidatooorAdapter] DataRelease: uniqueId=%s", uniqueId)
	req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("%s/data/release/%s", t.baseURL, uniqueId), nil)
	if err != nil {
		logger.Error("[TropidatooorAdapter] DataRelease: failed to create request: %v", err)
		return fmt.Errorf("failed to create request: %w", err)
	}

	resp, err := t.client.Do(req)
	if err != nil {
		logger.Error("[TropidatooorAdapter] DataRelease: failed to send request: %v", err)
		return fmt.Errorf("failed to send request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		logger.Error("[TropidatooorAdapter] DataRelease: non-200 status: %s", resp.Status)
		return fmt.Errorf("failed to release data for %s: status %s", uniqueId, resp.Status)
	}

	logger.Debug("[TropidatooorAdapter] DataRelease: success for uniqueId=%s", uniqueId)
	return nil
}
